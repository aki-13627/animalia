...
const petInputSchema = z.object({
  ...
  birthDay: z
    .string()
    .regex(/^\d{4}-\d{2}-\d{2}$/, { message: '誕生日はYYYY-MM-DD形式で入力してください' }),
...
export const RegisterPetModal: React.FC<RegisterPetModalProps> = ({
  ...
  const registerPetMutation = useMutation({
    mutationFn: (data: FormData) => {
      return axios.post('http://localhost:3000/pets/new', data, {
    ...
    <Modal animationType="none" transparent visible={visible} onRequestClose={onClose}>
      <View style={styles.modalOverlay}>
        ...
        >
          ...
          <Text style={[styles.modalTitle, { color: colors.text }]}>ペットを登録する</Text>
          <TouchableOpacity style={styles.iconContainer} onPress={pickIconImage}>
            {formData.iconImageUri ? (
              <Image source={{ uri: formData.iconImageUri }} style={styles.iconImage} />
            ) : (
              <Text style={[styles.iconPlaceholder, { color: colors.icon }]}>アイコン画像</Text>
            )}
          </TouchableOpacity>
          ...
          >
            <Text style={{ color: colors.text }}>{formData.petType === 'dog' ? '犬' : '猫'}</Text>
          </TouchableOpacity>
          ...
          <TextInput
            ...
            onChangeText={(value) => setFormData({ ...formData, birthDay: value})}
          />
          <TouchableOpacity
            onPress={handleSubmit}
            style={[styles.submitButton, { backgroundColor: colors.tint }]}
          >
            <Text style={{ color: colors.background, fontWeight: 'bold' }}>登録する</Text>
          </TouchableOpacity>
        </Animated.View>
        ...
        <Modal transparent visible={showPetTypeSelector} animationType="fade">
          ...
          >
            <View style={[styles.selectorContainer, { backgroundColor: colors.background }]}>
              <Text style={[styles.selectorTitle, { color: colors.text }]}>種類を選択</Text>
              <TouchableOpacity
                onPress={() => {
              ...
              >
                <Text style={[styles.selectorItem, { color: colors.text }]}>犬</Text>
              </TouchableOpacity>
              <TouchableOpacity
                onPress={() => {
              ...
              >
                <Text style={[styles.selectorItem, { color: colors.text }]}>猫</Text>
              </TouchableOpacity>
            </View>
          </TouchableOpacity>
        </Modal>
        <Modal transparent visible={showSpeciesSelector} animationType="fade">
          ...
          >
            <View style={[styles.selectorContainerFixed, { backgroundColor: colors.background }]}>
              <Text style={[styles.selectorTitle, { color: colors.text }]}>品種を選択</Text>
              <ScrollView>
                {speciesOptions[formData.petType].map((s) => (
                  ...
                  >
                    <Text style={[styles.selectorItem, { color: colors.text }]}>{s}</Text>
                  </TouchableOpacity>
                ))}
              </ScrollView>
            </View>
